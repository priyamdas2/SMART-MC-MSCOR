function [x_opt, fval, comp_time] = MSCORparallel(objFun, x0, MaxTime, MaxRuns, MaxIter, sInitial, rho, TolFun1, TolFun2, phi,lambda, DisplayUpdate, DisplayEvery, PrintStepSize, PrintSolution)
...
for iii = 1:MaxRuns
    ...
    for i = 1:MaxIter
        ...
        total_lh = zeros(2*B*M,1);
        matrix_update_at_h = zeros(2*B*M,M);
        
        total_lh_alt = zeros(2*B*M,1);
        matrix_update_at_h_alt = zeros(2*B*M,M);
        
        
        parfor index = 1:(2*B*M)
            b = ceil(index/(2*M));       % block number
            theta = Theta(b,:);
            temp_num = mod(index,2*M);
            if(temp_num == 0)
                temp_num = 2*M;
            end
            location_number = temp_num;  % location number
            ...
            if(M_here >= 2)
                ...
                if(D >= 0)
                    ...
                    total_lh(index) = objFun(Theta_temp);
                    total_lh_alt(index) = objFun(Theta_temp_2);
                else
                    possibility = theta;
                    possibility_alt = theta;
                    total_lh(index) = current_lh;
                    total_lh_alt(index) = current_lh;
                end
            else
            ...
            matrix_update_at_h_alt(index,:) = possibility_alt;
        end
        ...
        if(minValue_1 < min(minValue_2,current_lh))
            updated_block = matrix_update_at_h(linearIndex_1,:); % [block_no, full vec, location num]
            b = ceil(linearIndex_1/(2*M));
            Theta(b,:) = updated_block;
            final_value = minValue_1;
        else
            if(minValue_2 < current_lh)
                updated_block = matrix_update_at_h_alt(linearIndex_2,:);
                b = ceil(linearIndex_2/(2*M));
                Theta(b,:) = updated_block;
                final_value = minValue_2;
            else
                final_value = current_lh;
            end 
        end
        
        
        
        array_of_values(i) =  final_value;
...
fprintf('xxxxxxxxxxxxxxxxxxxxxx parallel-MSCOR ends xxxxxxxxxxxxxxxxxxxxxxxxxx\n')
end