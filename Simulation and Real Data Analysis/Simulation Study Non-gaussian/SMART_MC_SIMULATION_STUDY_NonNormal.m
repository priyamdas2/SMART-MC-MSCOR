...
dist = 1;          % X generated from: 1 = uniform(-10,10), 2 = Normal(0,1)
TrtSeqLengthEach = 20;
...
BetaCellTrue = BetaCellRandGenerator_2(RandSeed, N, p);
...
if(DoBootstrapNow == 1)
    ...
    filename = ['BetaALL_SIMULATION_BootStdErr_NonNormal_rep_',num2str(NumBootRep),'.mat'];
...
x0Mat = Generate_Rand_Initial_BetaMat_2(RandSeed, N, p);
...
filename = 'Output_Simulation_NonNormal_CoeffsEst.csv';
writematrix(round(CoeffsEst,2), filename);

filename = 'Output_Simulation_NonNormal_CoeffsTrue.csv';
...
filename = 'Output_Simulation_NonNormal_StdErr.csv';
