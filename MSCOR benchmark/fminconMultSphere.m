function [x0_opt_fmincon, fval_fmincon, comp_time] = fminconMultSphere(fun_asVec,x0,M,which_algo)
tic;
B = size(x0,1);
x0_vec = nan(B*M, 1);
for b = 1:B
...
[x_opt_vec, fval_fmincon] = fmincon(fun_asVec, x0_vec, [], [], [], [], [], [], nonlcon, options);

comp_time = toc;

x0_opt_fmincon = nan(B,M);
for b = 1:B
    x0_opt_fmincon(b,:) = x_opt_vec(((b-1)*M+1):(b*M));
end

fprintf('====================== fmincon starts ========================\n')
fprintf('\n');
if(which_algo == 1)
    fprintf('Algorithm used: interior-point\n');
end
if(which_algo == 2)
    fprintf('Algorithm used: sqp\n');
end
if(which_algo == 3)
    fprintf('Algorithm used: active-set\n');
end

fprintf('\n');
fprintf('function value at optimal solution: %d\n', fval_fmincon);
fprintf('xxxxxxxxxxxxxxxxxxxxxx fmincon ends xxxxxxxxxxxxxxxxxxxxxxxxxx\n')
