function TopHowManyNondiagTransFormula(TopHowMany, SeqCell, BetaCellFinal, BetaALLBootStdErr, N) 

CHat = TransitionCountMat(SeqCell, N);
CHatTrans = CHat(2:end,:);
CHatTransNonDiags = CHatTrans;
CHatTransNonDiags(logical(eye(size(CHatTransNonDiags)))) = 0;

[Values,ranks] = sort(CHatTransNonDiags(:),'descend');
